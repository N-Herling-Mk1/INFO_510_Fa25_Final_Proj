...
# -*- coding: utf-8 -*-
"""
sweep_model2_mk5m.py
───────────────────────────────────────────────────────────────────────────────
Full-Isolated Sweep System (with RRM Integration)
2025-11-30

Major Capabilities:
 • Each run stored in isolated folder.
 • Per-fold metrics, ROC curves, confusion matrix, latent correlation maps.
 • Bayesian latent regression metrics.
 • Full k-fold CV per config.
 • Live leaderboard tracking (accuracy + RRM).
 • Global running maxima for RRM.
 • Final leaderboard CSV/JSON saved.
 • Best run duplicated into leaders/ folder.
───────────────────────────────────────────────────────────────────────────────
"""
...
# sweep_model2_mk5m.py is located at: <project_root>/_code/models/
...
###############################################################################
# LOCAL MODULE IMPORTS (now guaranteed to work)
###############################################################################
...
# Local project imports with fallback auto-inference
...
###############################################################################
# CONFIG GENERATION — Updated Ranges
###############################################################################
    ...
    return [
        {
            ...
            "dropout_backbone": float(np.random.uniform(0.15, 0.35)),
            "dropout_tab":      float(np.random.uniform(0.10, 0.40)),
            "dense_units_img":  int(np.random.choice([256, 384, 512, 768])),
            "dense_units_tab":  int(np.random.choice([64, 128, 192, 256])),
            ...
            "kl_factor":        float(np.random.uniform(0.9, 1.5)),
            "prior_scale":      float(np.random.uniform(0.7, 1.5)),
            "posterior_scale_init": float(np.random.uniform(0.005, 0.025)),
        }
        for i in range(k)
    ]
...
###############################################################################
# BAYESIAN LATENT REGRESSION + POSTERIOR σ
###############################################################################
def run_bayesian(latents, labels_onehot, cfg_id, run_dir):
    ...
    # MEAN POSTERIOR σ for RRM
    posterior_sigma_mean = float(np.mean(sigma_all))
    return posterior_sigma_mean
...
###############################################################################
# RRM CALCULATION — OFFICIAL, LOCKED IN
###############################################################################
